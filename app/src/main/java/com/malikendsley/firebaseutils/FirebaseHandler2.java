...
public class FirebaseHandler2 {
    ...
    public void usernameToUID(String username, UsernameResolveListener listener) {
        mDatabase.child("TakenUsernames").child(username).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
    ...
    public void shareQuip(String recipientUID, byte[] byteArray, QuipUploadListener listener) {
        ...
        Log.i(TAG, "shareQuipd: Path: " + path);
                        ...
                        if (task.isSuccessful()) {
                            mDatabase.child("QuipsPrivate").child(publicQuipsReference.getKey()).setValue(privateQuip).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        //allclear
                                        Log.i(TAG, "shareQuip: Success - " + uri.toString());
    ...
    public void getReceivedFriendRequests(GetRequestsListener listener) {
        mDatabase.child("RequestsPrivate").child(mAuth.getUid()).child("Incoming").get().addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                ArrayList<String> l = new ArrayList<>();
                for (DataSnapshot child : task.getResult().getChildren()) {
    ...
    public void tryAddFriend(String username, FriendAddListener listener) {
        usernameToUID(username, resolvedUID -> {
            ...
            mDatabase.child("RequestsPrivate").child(mAuth.getUid()).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                @Override
                public void onComplete(@NonNull Task<DataSnapshot> task) {
                    ...
                    //just donwload all requests and check for the target UID, the number of requests will not scale
